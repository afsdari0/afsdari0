<div align="center">

# E aí, eu sou o Dario Ramos 👋

**Desenvolvedor Web** · Construindo para a web, do front ao back.

[![Portfolio](https://img.shields.io/badge/darioramos.dev-000?style=for-the-badge&logo=googlechrome&logoColor=4ecca3)](https://darioramos.dev)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-000?style=for-the-badge&logo=linkedin&logoColor=0a66c2)](https://www.linkedin.com/in/dario-ramos/)
[![WhatsApp](https://img.shields.io/badge/WhatsApp-000?style=for-the-badge&logo=whatsapp&logoColor=25d366)](https://wa.me/5582987560451)

</div>

---

### Sobre mim

Sou um desenvolvedor web com base sólida em front-end e visão de aplicação completa.
Trabalho com **Vue.js**, **Vuetify** e o ecossistema JavaScript — e venho expandindo minha atuação para **APIs**, banco de dados e lógica de servidor.

- 🟢 Disponível para projetos e freelance
- 🧩 Transformo ideias em produtos web completos — do layout responsivo à integração com serviços e dados
- 📚 Cada projeto é uma chance de ir além do que já sei

---

### Tech Stack

**Frontend**

![JavaScript](https://img.shields.io/badge/JavaScript-0d1117?style=flat-square&logo=javascript&logoColor=f7df1e)
![Vue.js](https://img.shields.io/badge/Vue.js-0d1117?style=flat-square&logo=vuedotjs&logoColor=4fc08d)
![Vuetify](https://img.shields.io/badge/Vuetify-0d1117?style=flat-square&logo=vuetify&logoColor=1867c0)
![SCSS](https://img.shields.io/badge/SCSS-0d1117?style=flat-square&logo=sass&logoColor=cc6699)
![HTML5](https://img.shields.io/badge/HTML5-0d1117?style=flat-square&logo=html5&logoColor=e34f26)
![CSS3](https://img.shields.io/badge/CSS3-0d1117?style=flat-square&logo=css3&logoColor=1572b6)

**Backend & Scripting**

![Python](https://img.shields.io/badge/Python-0d1117?style=flat-square&logo=python&logoColor=3776ab)

**Ferramentas & Workflow**

![Git](https://img.shields.io/badge/Git-0d1117?style=flat-square&logo=git&logoColor=f05032)
![Vite](https://img.shields.io/badge/Vite-0d1117?style=flat-square&logo=vite&logoColor=646cff)
![Pinia](https://img.shields.io/badge/Pinia-0d1117?style=flat-square&logo=vuedotjs&logoColor=ffd859)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-0d1117?style=flat-square&logo=githubactions&logoColor=2088ff)

---

### Projetos em Destaque

<table>
<tr>
<td width="50%">

**🤖 Tibia Bot** · `Rank A`
<br><br>
Automação de tarefas para Tibia com scripts em Python, foco em eficiência e controle. Inclui rotinas configuráveis e integração com o ambiente do jogo.
<br><br>
`Python` `Automação`
<br><br>
[![Ver no GitHub](https://img.shields.io/badge/Ver_no_GitHub-0d1117?style=flat-square&logo=github&logoColor=4ecca3)](https://github.com/afsdari0)

</td>
<td width="50%">

**🔐 Gerador de Senha** · `Rank C`
<br><br>
Aplicação web para gerar senhas fortes com opções de tamanho e caracteres. Interface simples, responsiva e pensada para uso rápido no dia a dia.
<br><br>
`JavaScript` `CSS`
<br><br>
[![Ver no GitHub](https://img.shields.io/badge/Ver_no_GitHub-0d1117?style=flat-square&logo=github&logoColor=4ecca3)](https://github.com/afsdari0)

</td>
</tr>
</table>

> 🔗 **Mais projetos em [darioramos.dev](https://darioramos.dev)**

---

### GitHub Stats
s
<div align="center">

<img height="170" src="https://github-readme-stats-ocue.vercel.app/api?username=afsdari0&show_icons=true&theme=github_dark&hide_border=true&bg_color=0d1117&title_color=4ecca3&icon_color=5b6fff&text_color=c9d1d9&ring_color=4ecca3&locale=pt-br" alt="GitHub Stats" />
&nbsp;&nbsp;
<img height="170" src="https://github-readme-stats-ocue.vercel.app/api/top-langs/?username=afsdari0&layout=compact&theme=github_dark&hide_border=true&bg_color=0d1117&title_color=4ecca3&text_color=c9d1d9&langs_count=6&locale=pt-br" alt="Linguagens Mais Usadas" />

</div>

---

<div align="center">

**Construindo para a web, do front ao back.** ⚡

<sub>Portfolio feito com Vue.js & Vuetify · Deploy via GitHub Pages</sub>

</div>
